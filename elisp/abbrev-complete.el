;;; abbrev-complete.el
;;;$Id: abbrev-complete.el,v 2.4 2005/12/23 01:33:45 akihisa Exp $
...
(defadvice expand-abbrev
  ...
  (cond
   ((string-match "\^M" (this-command-keys))
    ad-do-it)
...
(defun abbrev-complete-mode-quit ()
  ...
  (if (window-configuration-p abbrev-complete-windows-conf)
...
(defun abbrev-complete-car-expand ()
  (let (str tp empty (enable-abbrev-complete nil))
    (if (= (point-max) (point))
        (if (= (point-min) (point))
            (setq empty t)
          (put-text-property (- (point) 1) (point) 'abbcomp t))
...

(defvar enable-abbrev-complete t)
(defun abbrev-complete ()
  (interactive)
  (when enable-abbrev-complete
    (let* (pos word)
...
(add-hook 'pre-command-hook
          (lambda ()
            (if (and
                 abbrev-complete-windows-conf
                 (not (eq last-command this-command))
                 (string= "*Completions*"
                          (buffer-name (window-buffer (next-window)))))
                (abbrev-comple-delete-window))
            (setq abbrev-complete-windows-conf nil)))
